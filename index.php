...
<body>
    ...
    <script>
        ...
        $(document).on("keyup , focus", "#barcodeData tbody input", function(e) {
            ...
            if (e.keyCode == 13) {
                printBarcode($(dis).closest("tr"));
                return
                // $(dis).closest("td").next("td").find("[data-barcode]").select().focus();
                // if ($(dis).closest("td").next("td").find(".printdis").length) {
                //     alert()
                //     return
                //     printBarcode($(dis).closest("tr"));
                // }
                // else if (!$(dis).closest("td").next("td").find("[data-barcode]").length) {
                //     $(dis).closest("td").next("td").next("td").find("[data-barcode]").select().focus();
                // }
            } else if (e.keyCode == 38) {
                dis = $(dis).closest("tr").prev("tr");
                $(dis).closest("tr").find("td").find("[data-barcode]:first-child").select().focus();
                return
            } else if (e.keyCode == 40) {
                dis = $(dis).closest("tr").next("tr");
                $(dis).closest("tr").find("td").find("[data-barcode]:first-child").select().focus();
                return
            }
            var id = $(dis).closest("tr").attr("id");

            var val = conf.staticCode_value;
            $(dis).closest("tr").find("[data-barcode]").each(function() {
            ...
            $("#lablePrint thead .productCode").text(EXCEL[id][firstKey] || '------');
            $("#lablePrint thead .productTitle").text(EXCEL[id][secondKey] || '------');
            $("#lablePrint thead .productPrice").text($(dis).closest("tr").find("[data-barcode='Price_status']").val());
            $("#lablePrint thead .productWeight").text($(dis).closest("tr").find("[data-barcode='Weight_status']").val());
        ...
        $(document).ready(function() {
            $.getJSON("EXCEL.json?ver=" + Math.random(), function(data) {
                ...
                $.each(data, function(key, val) {
                    ...
                    clon.prop("id", key);
                    clon.find("td.productcode_status").append(val[firstKey] || '------').find("[data-barcode='productcode_status']").val(val[firstKey] || '------');
                    clon.find("td.productcode_title").text(val[secondKey] || '------');
                ...
                $('#barcodeData thead tr th.filterboxrow').each(function() {
                    ...
                    $(this).on('keyup change', function(e) {
                        // alert(e.keyCode)
                        if (e.keyCode == 13) {
                            if ($(this).closest("th").next("th").find("input.form-control").length) {
                                $(this).closest("th").next("th").find("input.form-control").focus()
                                return
                            }
                            $("#barcodeData tbody tr:first-child input[data-barcode='Weight_status']").focus();
                            return
                        } else if (e.keyCode == 39) {
                            $(this).closest("th").prev("th").find("input.form-control").focus()
                            return
